"""
OracleSurfer_v9 ("The Unchained Surfer")
- Based on v8 (EMA 100 + Ladder Exit).
- Key Change: REMOVED RSI Ceiling (<75) and Floor (>25).
  - Logic: If Oracle predicts a 4-day trend, we ignore "Overbought" signals.
    We allow entries into parabolic moves (RSI > 75).
- Tweak: Trailing Stop tightened to 1.0% (was 1.2%) to protect these riskier entries.
"""
...
class OracleSurfer_v9(IStrategy):
    """
    OracleSurfer_v9
    Uncapped Momentum: Trades parabolic moves if Oracle confirms.
    """
    ...
    # TRAILING STOP (MAXIMUM TIGHTNESS)
    # New: Trail by 1.0% (was 1.2%)
    # Reason: We are buying overbought tops; we need to exit FAST if they reject.
    trailing_stop = True
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # --- LONG ENTRY ---
        # REMOVED: (dataframe['rsi'] < 75)
        # We now enter even if RSI is 80, 90, etc.
        dataframe.loc[
            (
                (oracle_signal == "BULL") &
        ...
        # --- SHORT ENTRY ---
        # REMOVED: (dataframe['rsi'] > 25)
        # We now short even if RSI is 20, 10, etc.
        dataframe.loc[
            (
                (oracle_signal == "BEAR") &
                (dataframe['close'] < dataframe['ema_trend']) &
    ...
    def custom_stoploss(self, pair: str, trade: Trade, current_time: datetime,
        ...
        # Optimized Break-Even (Same as v8)
