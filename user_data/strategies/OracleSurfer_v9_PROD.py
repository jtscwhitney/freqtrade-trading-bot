"""
OracleSurfer_v9_PROD ("The Unchained Surfer")
Production Release
- Architecture: 1h Timeframe / Uncapped Momentum
- Key Characteristic: MAXIMUM AGGRESSION.
  This version explicitly IGNORES "Overbought" signals to trade parabolic breakouts.
  Best used during confirmed Bull Markets or "Altcoin Seasons."
- Logic:
    1. ENTRY FILTER (Safety): EMA 100 (Faster than v6/v10 to catch early trends).
    2. ENTRY TRIGGER (Aggression): Uncapped RSI (No ceiling).
...
class OracleSurfer_v9_PROD(IStrategy):
    """
    OracleSurfer_v9_PROD
    The "Bull Market Specialist".
    Use this when the market is trending hard and breaking new highs.
    """
    ...
    # ROI: UNCAPPED
    # We rely on the Tight Ladder to ride the parabola.
    minimal_roi = {
        "0": 100,       
        "240": 0.1,     
        "480": 0.05,    
    ...
    # HARD STOP: -5%
    # Crucial in this version because we buy tops.
    stoploss = -0.05
    
    # TRAILING STOP (TIGHT LADDER)
    # We need a hair-trigger exit because parabolic moves crash fast.
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # 2. TREND (Speed Layer)
        # EMA 100 (vs EMA 200 in v6/v10) allows earlier entries.
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # 3. Uncapped RSI (We buy even if RSI is 90!)
        # 4. Strong Momentum
        dataframe.loc[
            (
                (oracle_signal == "BULL") &
        ...
        # 1. Oracle BEAR
        # 2. Price < EMA 100
        # 3. Uncapped RSI (We sell even if RSI is 10!)
        dataframe.loc[
            (
                (oracle_signal == "BEAR") &
                (dataframe['close'] < dataframe['ema_trend']) &
    ...
    def custom_stoploss(self, pair: str, trade: Trade, current_time: datetime,
        ...
        # Since we are buying tops, we need to eliminate risk instantly.
        # Lock in profit at +1.2%.
